# --- NUEVA IMPLEMENTACIÓN IGUAL A SECCIÓN 7 ---
...
from scripts.extract_tables_of_excel import extract_tables_from_excel_verificacion_por_deflexion_horizontales
...
def verificacion_de_deflexión_horizontal_en_vigas(nuevo, idioma, file_path):
    """
    Genera la sección de verificación de deflexiones horizontales en vigas
    ...
    """
    print(f"[DEBUG] Generando sección 8 - Idioma: {idioma}, Archivo: {file_path}")
    
    key = 'en' if idioma.lower() in ('ingles', 'en') else 'es'
    textos = {
        'intro': {
            'en': "The horizontal deflection of the structure due to loads must be less than indicated in the project design criteria:",
            'es': "La deflexión horizontal de la estructura debido a las cargas debe ser menor a lo indicada en los criterios de diseño del proyecto:"
        },
        'titulo_sec': {
            'en': "Verification of horizontal deflections",
            'es': "Verificación de deflexiones horizontales"
        },
        'titulo_tab': {
            'en': "Table to verify horizontal deflection in beams",
            'es': "Tabla para verificar la deflexión horizontal en vigas"
        },
        'sin_datos': {
            'en': "No horizontal deflection verification data found.",
            'es': "No se encontraron datos de verificación de deflexión horizontal."
        }
    }
    ...
    # Extracción
    try:
        verif_dict = extract_tables_from_excel_verificacion_por_deflexion_horizontales(file_path)
    except Exception as e:
        print(f"Error extrayendo datos de Excel para sección 8: {e}")
    ...
    HEADERS_ES = [
        ...
        "Max DH (mm)", "Factor", "Deflexión Permisible L/X (mm)",
        "Cumple Norma"
    ]
    HEADERS_EN = [
        "ID", "Type", "Length (m)", "Load Case",
        "Max DH (mm)", "Factor", "Allowable Deflection L/X (mm)",
        "Standard Compliance"
    ]
    MAP_ES = {
        "ID": "ID",
    ...
    if len(available_headers) < 3:  # Mínimo 3 columnas para que tenga sentido
        print(f"Advertencia: Solo se encontraron {len(available_headers)} columnas válidas en sección 8")
    ...
    # Encabezados con letra tamaño 9
