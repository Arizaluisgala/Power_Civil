                ...
                for keyword in table_title_keywords:
                    if keyword.lower() in cell.value.lower():
                        ...
                        if key == 'en':
                            ...
                            title = re.sub(r'por viento', 'by Wind', title, flags=re.IGNORECASE)
...
def verificacion_de_desplazamientos_por_viento(nuevo,idioma,file_path):
    key = 'en' if idioma.lower() in ('ingles', 'en') else 'es'
    textos = {
        'titulo': {
            'en': "Verification of wind displacements",
            'es': "Verificación de desplazamientos por viento"
        },
        'intro': {
            'en': "The lateral deflection of the structure due to wind loads must be less than indicated in the project design criteria.",
            'es': "La deflexión lateral de la estructura debido a las cargas de viento debe ser menor a lo indicado en los criterios de diseño del proyecto."
        },
        'sin_datos': {
            'en': "No wind displacement verification data found.",
            'es': "No se encontraron datos de verificación de desplazamientos por viento."
        }
    }
    ...
    try:
        ...
        sheet = workbook['Viento']
    except (FileNotFoundError, KeyError) as e:
        print(f"Error opening Excel or sheet 'Viento': {e}")
